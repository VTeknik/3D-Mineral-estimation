...
[~,sheet_name]=xlsfinfo('SheikhPhase1.xlsx');
...
%   Area_Coordinates=xlsread('IP_Rs Data_SheikhdarAbad.xlsx',sheet_name{16});
  ...
  for ss=1:length(sheet_name)
%       ss=22
  %open file identifier
datain =  xlsread('SheikhPhase1.xlsx',sheet_name{ss}); % put each line one by one 
...
%   Array format change
  % =************************S-N********************************************
  % =************************S-N********************************************
  if mode (diff(datain(:,2)) )>0 % for profiles S-N                            
...
title([sheet_name{ss} '   S2N'])
pause(1)
 
 % x indicate the loctions along the profile; it can west-east or south-north
 xloc =[];
 data_out_i = [];topo_out = [];Xa= []; Ya= []; Za= []; Xm= []; Ym= []; Zm= []; Xn= []; Yn= []; Zn= [];rAM = [];rAN = [];
 ii = 1;
 for i = 1:length(ind)
     i
   xloc=[];yloc=[];a=[];n=[]; apres=[]; IP =[];k =[];
   
   
if i==length(ind) % for the last point you should count number of the points by using fig and write here
 ranges =ind(i):length(datain(:,2)); % here the last number indicate the number of the points minus 1
else
...
% ----------------------------------
 ...
 topo_out0 = [Xa' Ya' Za'; Xm(2:end) Ym(2:end) Zm(2:end) ; Xn(2:end) Yn(2:end) Zn(2:end)]; % location of all
 ytopo=topo_out0(:,2); ztopo=topo_out0(:,3);
 [~, rows] = unique(ytopo);
 ytopo = ytopo(rows);ztopo = ztopo(rows);
  [~, rows] = sort(ytopo);
 ytopo = ytopo(rows);ztopo = ztopo(rows);
 topo_out = [ytopo ztopo];
...
fprintf(fid_IPDC,fmt_SRC_topo,topo_out');
...
% =************************E-W********************************************
% =************************E-W********************************************
  if mode (diff(datain(:,1)) )>0 % for profiles E-W 
...
topo_out0 = [Xa' Ya' Za'; Xm(2:end) Ym(2:end) Zm(2:end) ; Xn(2:end) Yn(2:end) Zn(2:end)]; % location of all
 ytopo=topo_out0(:,1); ztopo=topo_out0(:,3);
 [~, rows] = unique(ytopo);
 ytopo = ytopo(rows);ztopo = ztopo(rows);
  [~, rows] = sort(ytopo);
 ytopo = ytopo(rows);ztopo = ztopo(rows);
